...
typedef struct angle_t{
  float h, v, logo, radiusMirrorBall, positionRedBall;
  int priorityAngle;
...
int counter = 0; 
bool isIncreasing = true, isGoingUp = true; 
...
main(int argc, char *argv[]) 
{
  ...
  {
    sf::Image result = tinyraytracer.render(0, 0, 15,-0.5,3);
...
void displayAndUserInteraction(Tinyraytracer tinyraytracer, bool animate)
{
  ...
  float angle_logo = 15.;
  float centerRedBall = -0.5, radiusMirror = 3.;
  sf::Clock clock;
  clock.restart();
  ...
  sf::Image starting_image = tinyraytracer.render(angle_v, angle_h, angle_logo, centerRedBall, radiusMirror);
  ...
  while (window.isOpen() && !exit_threadDisplayAndUserInteraction)
  {
    ...
    if (animate)
    {
      ...
      if (!angle_queue_too_big)
      {
        ...
        if(isGoingUp){
          centerRedBall += 2./5.;
          isGoingUp = centerRedBall >= 0.? false : true;
        }
        else{
          centerRedBall -= 2./5.;
          isGoingUp = centerRedBall <= -1.? true : false;
        }

        if(isIncreasing){
          radiusMirror += 2./3.;
          isIncreasing = radiusMirror >= 4.? false : true;
        }
        else{
          radiusMirror -= 2./3.;
          isIncreasing = radiusMirror <= 3.? true : false;
        }
        update = true;
        mutex.unlock();
      }
    ...
    if (update)
    {
      ...
      angle.positionRedBall = centerRedBall;
      angle.radiusMirrorBall = radiusMirror;
...
void computationImage(Tinyraytracer tinyraytracer)
{
  ...
  while(!exit_computation_threads)
  {
    ...
    { 
      ...
      sf::Image image_render = tinyraytracer.render(current_angles.v, current_angles.h, current_angles.logo, current_angles.positionRedBall, current_angles.radiusMirrorBall);
