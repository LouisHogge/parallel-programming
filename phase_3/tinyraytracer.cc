...
sf::Image
Tinyraytracer::render(float anglev, float angleh, float anglel, float positionBall, float radiusBall)
{
  this->varyCenter(positionBall);
  this->varyRadius(radiusBall);
  ...
  { 
    for (size_t i = 0; i < width; i++) 
    {
      ...
      for (size_t k = 0; k < 3; k++)
        pixmap[(j * width + i) * 4 + k] = (unsigned char)(255 * std::max(0.f, std::min(1.f, f[k])));
...
void Tinyraytracer::varyCenter(float center){
    mtx.lock();
    this->spheres[1].center[1] = center;
    mtx.unlock();
}

void Tinyraytracer::varyRadius(float radius){
    mtx.lock();
    this->spheres[2].radius = radius;
    mtx.unlock();
}
